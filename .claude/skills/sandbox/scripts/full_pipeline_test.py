...
async def test_openai_pipeline():
    ...
    messages = [
        {
            "role": "user",
            "content": "Create a dinner appointment for tomorrow at 7 PM with John at Restaurant XYZ",
        }
    ]
...
async def test_openai_thinking_pipeline():
    ...
    response = await agent.arun(
        messages=messages, model="gpt-5", response_format="plain", user_name="Test User"
    )
