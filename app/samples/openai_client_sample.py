...
async def test_structured_output(client: OpenAIClient, model: str):
    ...
    print(
        f"LLM trace: model={parsed.llm_trace.model}, in={parsed.llm_trace.input_tokens}, out={parsed.llm_trace.output_tokens}"
    )
    return response
...
async def test_function_with_structured(client: OpenAIClient, model: str):
    ...
    messages = [
        {
            "role": "system",
            "content": "You are a helpful assistant. Create an event for the user.",
        },
        {
            "role": "user",
            "content": "Schedule a meeting tomorrow at 3pm called 'Team Sync'",
        },
    ]
    response = await client.create_completion(
        messages=messages,
