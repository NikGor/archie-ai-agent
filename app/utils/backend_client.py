...
class BackendClient:
    ...
    async def create_conversation(
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                logger.info(
                    f"backend_003: Created conversation: \033[32m{created_conversation_id}\033[0m"
                )
                return created_conversation_id

        except Exception as e:
            logger.error(
                f"backend_error_001: Conversation creation failed: \033[31m{e!s}\033[0m"
            )
            raise
    ...
    async def create_message(self, message: ChatMessage) -> ChatMessage:
        ...
        try:
            ...
            logger.info(
                f"backend_005: Message ID: \033[36m{message.message_id or 'AUTO'}\033[0m"
            )
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                logger.info(
                    f"backend_006: Created message: \033[32m{message.message_id}\033[0m"
                )
                return message

        except Exception as e:
            logger.error(
                f"backend_error_002: Message creation failed: \033[31m{e!s}\033[0m"
            )
            raise
    ...
    async def get_messages(
        ...
        try:
            ...
            if conversation_id:
                ...
                logger.info(
                    f"backend_008: Conversation filter: \033[36m{conversation_id}\033[0m"
                )
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                logger.info(
                    f"backend_009: Retrieved messages: \033[33m{len(messages)}\033[0m"
                )
                return messages

        except Exception as e:
            logger.error(
                f"backend_error_003: Message retrieval failed: \033[31m{e!s}\033[0m"
            )
            return []
